...
from boto_scripts.create_instance_old import create_instance_old
...
from boto_scripts.read_script import read_script
from botocore import create_vpc
...
def deploy():
    ...
    # Create db instance
    db_ip = create_instance_old(keyname, 1, 1)
    ...
    send_cmd(
        ssh, 'echo "mongo generate_data.mongo.js" | sudo docker exec -i some-mongo bash -')
    close_ssh(ssh)
    ...
    load_balancer_api = create_load_balancer(load_balancer_security_group.id, [
        # TODO: Double check how we will get the subnets to pass in here
        'subnet-073cd7a90757fd3a4',
        'subnet-0e7ef3710998198bc',
    ])
    target_group_api = create_target_group(vpc_id)
    registered_target1_api = register_ec2_instance_with_target_group(list(target_group_api.values())[0][0].get(
        'TargetGroupArn'), 'i-008b7ba987ee8d6ea')  # TODO: Replace ec2 instance ID here with one we generate
    registered_target2_api = register_ec2_instance_with_target_group(list(target_group_api.values())[0][0].get(
        'TargetGroupArn'), 'i-0ce79f06373a78937')  # TODO: Replace ec2 instance ID here with one we generate
    listener_api = create_listener(list(target_group_api.values())[0][0].get(
        'TargetGroupArn'), list(load_balancer_api.values())[0][0].get('LoadBalancerArn'))
    load_balancer_api_dns = list(load_balancer_api.values())[
        0][0].get('DNSName')
    ...
    # Temporary testing with my VPC_ID, will use vpc_id we generate
    target_group_ui = create_target_group(vpc_id)
    registered_target1_ui = register_ec2_instance_with_target_group(list(target_group_ui.values())[0][0].get(
        'TargetGroupArn'), 'i-008b7ba987ee8d6ea')  # TODO: Replace ec2 instance ID here with one we generate
    registered_target2_ui = register_ec2_instance_with_target_group(list(target_group_ui.values())[0][0].get(
        'TargetGroupArn'), 'i-0ce79f06373a78937')  # TODO: Replace ec2 instance ID here with one we generate
    listener_ui = create_listener(list(target_group_ui.values())[0][0].get(
        'TargetGroupArn'), list(load_balancer_ui.values())[0][0].get('LoadBalancerArn'))
